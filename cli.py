"""
Command Line Interface for Hrd Parp Triager Agent.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="hrd-parp-triager-agent", description="Hrd Parp Triager Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  HRD PARP TRIAGER AGENT")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Hrd Parp Triager Agent Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "batch":
        ...
        # Path traversal protection: ensure input exists and is a file
        if not input_path.is_file():
            print(f"Error: Input file not found: {input_path}", file=sys.stderr)
            return 1

        # Prevent path traversal in output: write only to current working directory or system temp
        cwd = Path.cwd().resolve()
        tmp = Path(tempfile.gettempdir()).resolve()
        if not (str(output_path).startswith(str(cwd)) or str(output_path).startswith(str(tmp))):
            print(f"Error: Output path must be within the current working directory or system temp. Got: {output_path}", file=sys.stderr)
            return 1
        ...
        for idx, r in enumerate(rows):
            try:
                payload = SystemTaskPayload(
                    task_id=r.get("task_id", f"TASK-{idx:04d}"),
    ...
    if args.command == "serve":
        ...
        print(f"Starting Hrd Parp Triager Agent API server on http://{args.host}:{args.port}")
