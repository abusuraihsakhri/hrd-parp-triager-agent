"""
Pydantic v2 schemas and data definitions for Hrd Parp Triager Agent.
...
class SystemTaskPayload(BaseModel):
    ...
    def _reject_nan_inf(cls, v: float) -> float:
        if math.isnan(v) or math.isinf(v):
            raise ValueError("Metric values must be finite numbers (NaN/Inf rejected)")
        return v
...
class ConsensusDossier(BaseModel):
    dossier_id: str
    system_slug: str = "hrd-parp-triager-agent"
